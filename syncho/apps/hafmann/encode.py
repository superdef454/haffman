...
def haffman_in_json(data: str):
    shifr = {
                "alphabet": {},
                "shifr" : ""
            }
    code = huffman_encode(data)  # кодируем строку
    encoded = "".join(code[ch] for ch in data)  # отобразим закодированную версию, отобразив каждый символ
    for ch in sorted(code): # обойдем символы в словаре в алфавитном порядке с помощью функции sorted()
        shifr["alphabet"][ch] = code[ch]  # выведем символ и соответствующий ему код
    shifr["shifr"] = str(encoded)  # выведем закодированную строку
    return shifr